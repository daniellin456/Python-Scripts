...
def plot_equilibrium_temperature(fig, ax, nH_array, balance_temperature):
    ax.loglog(nH_array, balance_temperature, linewidth=3, linestyle="--", color="black")
    return


def plot_m(fig, ax, nH_mesh, T_mesh, extent, m_matrix):
    ...
    sc = ax.scatter(nH_mesh, T_mesh, c=m_matrix.T, cmap="rainbow", norm=LogNorm(vmin=1e-1, vmax=1e2))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_title("m(nH, T)")
    ax.set_xlabel(r'$\rm{log}\; nH \; (\rm{cm^{-3}})$')
    ax.set_ylabel(r'$\rm{log}\; T \; (\rm{K})$')
    ax.set_xlim(np.min(nH_mesh), np.max(nH_mesh))
    ax.set_ylim(np.min(T_mesh), np.max(T_mesh))

    fig.colorbar(sc, ax=ax, extend='both', label='m')
    return


def plot_n(fig, ax, nH_mesh, T_mesh, extent, n_matrix):
    ...
    sc = ax.scatter(nH_mesh, T_mesh, c=n_matrix.T, cmap="rainbow", norm=LogNorm(vmin=1e-2, vmax=1e2))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_title("n(nH, T)")
    ax.set_xlabel(r'$\rm{log}\; nH \; (\rm{cm^{-3}})$')
    ...
    ax.set_xlim(np.min(nH_mesh), np.max(nH_mesh))
    ax.set_ylim(np.min(T_mesh), np.max(T_mesh))
    fig.colorbar(sc, ax=ax, extend='both', label='n')
    return


def plot_Gamma1(fig, ax, nH_mesh, T_mesh, extent, Gamma1):
    ...
    sc = ax.scatter(nH_mesh, T_mesh, c=Gamma1.T, cmap="rainbow", norm=SymLogNorm(linthresh=1, vmin=-10, vmax=10))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax.set_xlim(np.min(nH_mesh), np.max(nH_mesh))
    ax.set_ylim(np.min(T_mesh), np.max(T_mesh))
    fig.colorbar(sc, ax=ax, extend="both", label=r"$\Gamma_1$")
    return


def plot_Gamma2(fig, ax, nH_mesh, T_mesh, extent, Gamma2):
    ...
    sc = ax.scatter(nH_mesh, T_mesh, c=Gamma2.T, cmap="rainbow", norm=SymLogNorm(linthresh=1, vmin=-10, vmax=10))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax.set_xlim(np.min(nH_mesh), np.max(nH_mesh))
    ax.set_ylim(np.min(T_mesh), np.max(T_mesh))
    fig.colorbar(sc, ax=ax, extend="both", label=r"$\Gamma_2$")
    return


def plot_Gamma3(fig, ax, nH_mesh, T_mesh, extent, Gamma3):
    print("Extrema of Gamma_3: min: %24.14e, max: %24.14e" % (np.min(Gamma3), np.max(Gamma3)))
    length = np.sqrt(2 + Gamma3.T ** 2 - 2 * Gamma3.T)
    sc = ax.scatter(nH_mesh, T_mesh, c=Gamma3.T, cmap="rainbow", norm=SymLogNorm(linthresh=0.01, vmin=-10, vmax=10))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax.set_xlim(np.min(nH_mesh), np.max(nH_mesh))
    ax.set_ylim(np.min(T_mesh), np.max(T_mesh))
    fig.colorbar(sc, ax=ax, extend="both", label=r"$\Gamma_3$")
    return


def plot_Gamma1_m1(fig, ax, nH_mesh, T_mesh, Gamma1_m1):
    step = 25
    length = np.sqrt(Gamma1_m1.T ** 2 + 1)
    ax.quiver(nH_mesh[::step, ::step], T_mesh[::step, ::step], (1 / length)[::step, ::step],
              (Gamma1_m1.T / length)[::step, ::step], color="white")
    return


def plot_Gamma2_m1(fig, ax, nH_mesh, T_mesh, Gamma2_m1):
    step = 25
    length = np.sqrt(Gamma2_m1.T ** 2 + 1)
    ax.quiver(nH_mesh[::step, ::step], T_mesh[::step, ::step], (1 / length)[::step, ::step],
              (Gamma2_m1.T / length)[::step, ::step], color="white")
    return


def plot_Gamma3_m1(fig, ax, nH_mesh, T_mesh, Gamma3_m1):
    step = 25
    length = np.sqrt(Gamma3_m1.T ** 2 + 1)
    ax.quiver(nH_mesh[::step, ::step], T_mesh[::step, ::step], (1 / length)[::step, ::step],
              (Gamma3_m1.T / length)[::step, ::step], color="white")
    return
...
def make_plot(nH_array, T_array, balance_temperature, m_matrix, n_matrix, Gamma1, Gamma2, Gamma3):
    ...
    plot_m(fig, ax0, nH_mesh, T_mesh, extent, m_matrix)
    plot_equilibrium_temperature(fig, ax0, nH_array, balance_temperature)

    plot_n(fig, ax1, nH_mesh, T_mesh, extent, n_matrix)
    plot_equilibrium_temperature(fig, ax1, nH_array, balance_temperature)

    plot_Gamma1(fig, ax3, nH_mesh, T_mesh, extent, Gamma1)
    plot_equilibrium_temperature(fig, ax3, nH_array, balance_temperature)

    plot_Gamma2(fig, ax4, nH_mesh, T_mesh, extent, Gamma2)
    plot_equilibrium_temperature(fig, ax4, nH_array, balance_temperature)

    plot_Gamma3(fig, ax5, nH_mesh, T_mesh, extent, Gamma3)
    plot_equilibrium_temperature(fig, ax5, nH_array, balance_temperature)
