...
#    hp = rc**1.5 * cs / boxlen / np.sqrt(mass_sink)  ## disc scale height from hydrostatic equilibrium
    ...
    def plot_reso(f,ylim): ## add resolution limit to plot
        plt.figure(f.number)
        plt.plot(np.ones(2)*dx0*len_AU, ylim,'k',zorder=0,alpha=0.1)
        plt.plot(np.ones(2)*4*dx0*len_AU, ylim,'k',zorder=0,alpha=0.1)
...
#    Sig_params = curve_fit(func_Sig,rc[4:-npad],np.log(Sigma_sel[4:-npad]),p0=(Scrit,Rcrit*1.,sa,sb,1,5.e-3))[0]
    ...
    def plot_Rk_Rp(f, ylim):
        plt.figure(f.number)
        plt.plot(np.ones(2)*Rk*len_AU, ylim,ls=':',c='C8',zorder=1)
        plt.plot(np.ones(2)*Rp*len_AU, ylim,ls=':',c='C8',zorder=1)
    if f_Sigma:
...
#### some disk radial internal properties #### 
    ...
    def calc_snowlines(func_r_T, Ts):
        ...
        return Rsnow
    def plot_snowlines(f, ylim, Ts,Rsnow):
        plt.figure(f.number)
        for T, r in zip(Ts,Rsnow):
            if r==0.: continue
            plt.plot(np.ones(2)*r*len_AU, ylim,ls='--',c='C9',lw=1,zorder=1)
            plt.plot(np.array([xmin,xmax]),np.ones(2)*T,ls='--',c='C9',lw=1,zorder=1)
    ...
    if f_T: ## plot temperature
        ...
        plt.legend()
        #plt.savefig(path_out+'/disk_T_'+str(ioutput).zfill(5)+'.pdf',bbox_inches='tight')
