...
if __name__ == '__main__':
    ...
    os.chdir(basedir)
    print "args.readdir"+"*"
    runs = glob.glob(args.read_dir+"*")
    print runs
    f = open(writedir+"/times.txt","w")

    for run in runs:
        readdir, writedir = local_dir(run)
        print readdir
        outputs = search_ro(readdir)
        nout = len(outputs)
        #print outputs[:10]
        if run == 'DC_2_fld_rest3_hres_hsink': outputs[0]=3384
        ro = RamsesOutput(readdir,outputs[0],order='<') 
        time_year = ro.info["unit_time"].express(C.year)
        time_i = ro.info["time"]*time_year
        ro = RamsesOutput(readdir,outputs[-1],order='<')
        time_f = ro.info["time"]*time_year
        print run, time_i, time_f
        f.write("     ".join(map(str, [run, outputs[0], outputs[-1], time_i, time_f]))+"\n")
    f.close()
#    outputs = search_ro(readdir)
#    nout = len(outputs)
